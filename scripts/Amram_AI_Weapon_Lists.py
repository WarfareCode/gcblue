...
def HasRockets(interface):
    ...
    if group:
        for unit in xrange(interface.GetUnitCount()):
            ...
            nLaunchers = UI.GetLauncherCount()
            for n in range(0, nLaunchers):
                if UI.QueryDatabase('ballistic',UI.GetLauncherWeaponName(n),'BallisticType').GetRow(0).GetString(0) == '5':  #rocket
                    return 1
        return 0
    else:
        UI = interface
...
def HasGBU(interface):
    ...
    if group:
        for unit in xrange(interface.GetUnitCount()):
            ...
            nLaunchers = UI.GetLauncherCount()
            for n in range(0, nLaunchers):
                if UI.QueryDatabase('ballistic',UI.GetLauncherWeaponName(n),'BallisticType').GetRow(0).GetString(0) == '3':  #guided bomb
                    return 1
        return 0
    else:
        UI = interface
...
def HasStrafeGuns(interface):
    ...
    if group:
        for unit in xrange(interface.GetUnitCount()):
            ...
            for n in range(0, nLaunchers):
                ...
                if (UI.QueryDatabase('ballistic',weapon_name,'BallisticType').GetString(0) in ['0','2'] and
                    (target_flags(int(UI.QueryDatabase('ballistic', weapon_name, 'targetFlags').GetRow(0).GetString(0)),1) or
                    target_flags(int(UI.QueryDatabase('ballistic', weapon_name, 'targetFlags').GetRow(0).GetString(0)),4))):
                    return 1
        return 0
    else:
        UI = interface
        ...
        for n in range(0, nLaunchers):
            weapon_name = UI.GetLauncherWeaponName(n)
            if (UI.QueryDatabase('ballistic',weapon_name,'BallisticType').GetRow(0).GetString(0) in ['0','2'] and
                (target_flags(int(UI.QueryDatabase('ballistic', weapon_name, 'targetFlags').GetRow(0).GetString(0)),1) or
                target_flags(int(UI.QueryDatabase('ballistic', weapon_name, 'targetFlags').GetRow(0).GetString(0)),4))):
                return 1
        return 0
...
def HasAAW(interface):
    ...
    if group:
        for unit in xrange(interface.GetUnitCount()):
            if target_flags(int(UI.QueryDatabase('missile', UI.GetLauncherWeaponName(n), 'targetFlags').GetRow(0).GetString(0)),2):
                return 1
        return 0
    else:
        UI = interface
...
def strafe_capable(UI):
    ...
    for n in range(0, UI.GetLauncherCount()):
        ...
        if (UI.QueryDatabase('ballistic',weapon_name,'BallisticType').GetRow(0).GetString(0) in ['0','2','5'] and
            (target_flags(int(UI.QueryDatabase('ballistic', weapon_name, 'targetFlags').GetRow(0).GetString(0)),1) or
            target_flags(int(UI.QueryDatabase('ballistic', weapon_name, 'targetFlags').GetRow(0).GetString(0)),4))):
            capable = True
    return capable        
