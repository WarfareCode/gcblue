...
public:
	Sky();
	virtual ~Sky();
	void update(double lat, double lon, DateZulu const &);
	osg::Light* getSunLight();
	osg::Light* getMoonLight();
    osg::Drawable* getHorizon();
...
protected:
    ...
    double m_LastStarsUpdate;
	double m_LMST; ///< Local sidereal time in radians http://www.faqs.org/faqs/astronomy/faq/part3/section-5.html
	double m_Latitude;
	double m_SpinTheWorld;

	static float skyDomeRadius; ///< radius for sky dome
};
