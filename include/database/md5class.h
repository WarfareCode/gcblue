...
public:
	...
	CMD5(const std::string& plainText);
    ...
    const char* getMD5Digest() const;	  //access message digest (aka hash), return 0 if plaintext has not been set
    bool isDigestValid() const;
	bool operator==(const CMD5& rhs);

	virtual ~CMD5();
private:
    enum {DIGEST_STRING_SIZE=33};
	...
	char m_digestString[DIGEST_STRING_SIZE];  //Null terminated string value of the digest expressed in hex digits
